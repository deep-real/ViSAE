...
class BatchTopKSAE(Dictionary, nn.Module):
    ...
    # def encode(self, x: t.Tensor, return_active: bool = False, use_threshold: bool = True, preprocess_input: bool = True):
    #     if preprocess_input:
    ...
    def encode(self, x: t.Tensor, return_active: bool = False, use_threshold: bool = True, preprocess_input: bool = True, return_raw: bool = False):
        if preprocess_input:
            x, x_mean, x_std = self.preprocess_input(x)
            self.x_mean = x_mean
            self.x_std = x_std
        ...
        if return_raw:
            # Return raw ReLU activations without any thresholding or top-k
    ...
    def decode(self, x: t.Tensor, postprocess_output: bool = True) -> t.Tensor:
        x_reconstruct = self.decoder(x) + self.b_dec
...
class BatchTopKTrainer(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        x, _, _ = self.ae.preprocess_input(x)
        f, active_indices_F, post_relu_acts_BF = self.ae.encode(
            x, return_active=True, use_threshold=False, preprocess_input=False
        )
        ...
        x_hat = self.ae.decode(f, postprocess_output=False)
