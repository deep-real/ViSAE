...
class AutoEncoder(Dictionary, nn.Module):
    ...
    def encode(self, x, preprocess_input:bool=True):
        if preprocess_input:
    ...
    def decode(self, f, postprocess_output:bool=True):
        x_reconstruct = self.decoder(f) + self.bias
        if postprocess_output:
            return self.postprocess_output(x_reconstruct, self.x_mean, self.x_std)
        else:
            return x_reconstruct

    def forward(self, x, output_features=False, ghost_mask=None, normalize_input:bool=True):
        """
        ...
        if ghost_mask is None:  # normal mode
            f = self.encode(x, preprocess_input=normalize_input)
            x_hat = self.decode(f, postprocess_output=normalize_input)
            if output_features:
                return x_hat, f
            else:
                return x_hat
...
class GatedAutoEncoder(Dictionary, nn.Module):
    ...
    def encode(self, x: t.Tensor, return_gate:bool=False, normalize_decoder:bool=False, preprocess_input:bool=True):
        """
    ...
    def decode(self, f: t.Tensor, normalize_decoder:bool=False, postprocess_output:bool=True):
        if normalize_decoder:
            # If the SAE is trained without ConstrainedAdam, the decoder vectors are not normalized
            # Normalizing after encode, and renormalizing before decode to enable comparability
            f = f / self.decoder.weight.norm(dim=0, keepdim=True)
        x_reconstruct = self.decoder(f) + self.decoder_bias
        if postprocess_output:
            return self.postprocess_output(x_reconstruct, self.x_mean, self.x_std)
        else:
            return x_reconstruct
...
class JumpReluAutoEncoder(Dictionary, nn.Module):
    ...
    def encode(self, x, output_pre_jump=False, preprocess_input:bool=True):
        if preprocess_input:
            x, x_mean, x_std = self.preprocess_input(x)
            self.x_mean = x_mean
            self.x_std = x_std
